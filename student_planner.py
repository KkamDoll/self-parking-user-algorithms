...
PARAMS = {
    ...
    "v_cruise": 2.0,   # 일반 주행 목표 속도(m/s) — target_speed 기본값(곡률 미지정 시)
    # [G] 곡률 적응 속도: 직선은 빠르게(가속↑), 곡선/주차기동은 느리게(추종오차↓ → 충돌↓)
    "v_straight": 4.0,   # 직선(저곡률) 목표 속도(m/s). 직선 가속 강화(곡선은 자동 감속)
    ...
    "v_min": 0.5,      # 크롤 최저 속도
    "k_goal": 0.8,     # 골까지 거리 1m당 허용 속도 증가량
    "k_cusp": 1.1,     # 전/후진 전환점(컵스)까지 거리 1m당 허용 속도 증가량
...
def lock_stop_decision(x, y, yaw, slot, goal, best_iou,
                       deep_iou=0.45, center_tol=0.45, drop=0.02):  # [I] 깊이-진입 인지 락-스톱
    """슬롯에 '끝까지' 들어선 뒤 정지하도록 정지 시점을 잡는다.

    IoU 는 차가 슬롯에 완전히 들어가면 정점(~0.52)에서 평탄해진다(더 들어가도 그대로). 따라서
    'IoU 임계 하나'만으론 진입 도중(차 뒤가 슬롯 밖으로 삐져나온 얕은 자세)에 멈춰 버린다.
    그래서 두 조건을 함께 본다:
      · 충분히 깊다: 현재 IoU >= deep_iou(0.45)  그리고
      · (a) 슬롯 중앙(goal)에 center_tol(0.45m) 이내로 도달했거나,
        (b) 정점을 지나 빠져나가기 시작(IoU 가 best 에서 drop 이상 하락)
    둘 중 하나면 정지 → 차가 박스 안에 완전히 들어와 가운데 정렬된 자세에서 멈춘다.
    반환 (정지?, 현재IoU). (기존 0.40 단일임계 조기정지 문제 해결)
    """
    if slot is None or not orientation_ok(yaw, slot):
        return False, 0.0
    iou = slot_iou(_car_true_corners(x, y, yaw), slot)
    if iou < deep_iou:
        return False, iou                       # 아직 얕게 걸침 -> 더 깊이 진입
    centered = (goal is not None
                and math.hypot(x - goal[0], y - goal[1]) < center_tol)
    if centered or iou <= best_iou - drop:
        return True, iou
    return False, iou
            ...
            if not self._planning:              # 계획 스레드 시작 (최초 1회)
                ...
                def _worker(gen=gen, map_data=self.map_data, start_state=start_state,
                    ...
                    def eval_path(p, goal_c):
                        ...
                        #     선택 키 최상위에 둬, 검증 통과한 '요구방향' 경로를 (cusp/길이가 조금
                        #     더 들더라도) 반대방향보다 우선 채택 → orientation 점수 확보.
                    ...
                    if gen == self._plan_gen:    # 최신 요청일 때만 반영 (맵 교체 race 방지)
                        if chosen is not None:
                            ...
                            ori = "요구방향" if chosen[0][0] == 0 else "반대방향"
                            print(f"[H] 경로 채택: {tag}/{ori} cusps={chosen[0][1]} len={chosen[0][2]}")
