...
PARAMS = {
    # [I] 속도 최적화(2026-06-17): 평균속도 0.6m/s(목표 2.0~3.2)로 시간·속도 점수가 폭락하던 문제.
    #     목표속도를 전반적으로 상향. 안전은 (a) 곡률적응속도가 곡선/주차기동에서 자동 감속,
    #     (b) cte 감속(참 수직거리 기반)이 경로 이탈 시 감속, (c) simulate_follow 롤아웃 검증이
    #     '실제로 충돌 없이 주차되는' 경로만 채택 — 세 겹의 안전망이 빠른 속도를 받쳐준다.
    "v_cruise": 3.0,   # 일반 주행 목표 속도(m/s) — target_speed 기본값(곡률 미지정 시)
    # [G] 곡률 적응 속도: 직선은 빠르게(가속↑), 곡선/주차기동은 느리게(추종오차↓ → 충돌↓)
    "v_straight": 6.0,   # 직선(저곡률) 목표 속도(m/s). 직선 가속 강화(곡선은 자동 감속)
