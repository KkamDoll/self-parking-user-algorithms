...
def lock_stop_decision(x, y, yaw, slot, best_iou, deep_iou=0.45, drop=0.03):  # [I] 깊이-진입 인지 락-스톱
    """슬롯에 '끝까지' 들어선 뒤 정지하도록, 다음 두 경우에만 정지를 지시한다.
      (1) 깊이 진입: 현재 IoU >= deep_iou(0.45) — 슬롯 중앙(IoU 최대 ~0.52)에 가깝게 들어옴.
      (2) 정점 통과: 이미 충분히 깊었고(best_iou>=0.40) IoU 가 정점에서 drop 이상 떨어짐
          → 더 들어가면 오히려 빠져나가므로, 도달 가능한 가장 깊은 자세에서 정지.
    반환 (정지?, 현재IoU). 기존 0.40 단일임계는 얕게 걸친 자세에서 조기정지(차가 박스 밖으로
    삐져나온 채 멈춤)했는데, 이를 '깊이 + 정점' 기준으로 바꿔 완전히 진입한 뒤 멈춘다.
    best_iou>=0.40 게이트는 과거 0.40 락의 성공 보장을 유지한다(0.40 도달분은 여전히 정지).
    """
    if slot is None or not orientation_ok(yaw, slot):
        return False, 0.0
    ...
    if iou >= deep_iou:
        return True, iou
    if best_iou >= 0.40 and iou <= best_iou - drop:
        return True, iou
    return False, iou
...
def simulate_follow(path, start_state, goal, collision_free, limits, max_steps=12000, slot=None):  # [H]
    ...
    for _ in range(max_steps):
        ...
        lock_r, cur_iou_r = (lock_stop_decision(x, y, yaw, slot, best_iou_roll)
...
class PlannerSkeleton:
    ...
    def _compute_control_D(self, obs: Dict[str, Any]):
        ...
        lock_now, cur_iou = lock_stop_decision(state["x"], state["y"], state.get("yaw", 0.0),
                                               self._target_slot, self._best_iou)
