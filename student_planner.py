...
PARAMS = {
    # [I] 속도 최적화: 평균속도 0.6m/s(목표 2.0~3.2)로 시간·속도 점수가 폭락하던 문제. 목표속도 상향.
    #     안전망 3겹: (a)곡률적응속도 자동감속 (b)cte 감속(참 수직거리) (c)롤아웃 검증.
    "v_cruise": 1.5,   # 일반 주행 목표 속도(m/s) — target_speed 기본값(곡률 미지정 시)
    # [G] 곡률 적응 속도: 직선은 빠르게(가속↑), 곡선/주차기동은 느리게(추종오차↓ → 충돌↓)
    # [I] v_straight 4.0→2.5: 4.0은 추종 불가 속도라 롤아웃 검증이 전부 실패(→느린 폴백)했다.
    #     추종 가능 속도로 낮춰 검증을 통과시키면 '검증된 매끄러운 경로'를 빠르고 안전하게 쓴다.
    "v_straight": 2.5,   # 직선(저곡률) 목표 속도(m/s). 직선 가속 강화(곡선은 자동 감속)
    "v_curve_min": 1.0,  # 급곡선(최소회전반경급) 목표 속도(m/s)
    # [G] 추종오차 기반 감속: 경로에서 벗어날수록 느리게 -> 좁은 곳 충돌 전 회복 시간 확보
    # [I] cte 는 path_cross_track(참 수직거리)로 측정 → 직선에선 cte≈0 이라 감속 거의 없음.
    "cte_slow_gain": 1.0,   # 추종오차 1m 당 속도 상한 감소 비율
    "cte_slow_floor": 0.5,  # 최소 속도 비율(이 이하론 안 줄임)
    "v_min": 0.4,      # 크롤 최저 속도 [I] 0.5→0.4: 골 근처 확실히 멈추도록(검증/충돌 안정)
    "v_unverified_cap": 1.3,  # [I] 미검증 폴백 경로 추종 시 속도 상한(보수적, 충돌 방지)
    "k_goal": 0.6,     # 골까지 거리 1m당 허용 속도 증가량 [I] 0.8→0.6: 골 접근 감속 복원
    "k_cusp": 0.8,     # 전/후진 전환점(컵스)까지 거리 1m당 허용 속도 증가량 [I] 1.1→0.8 복원
    "kp_v": 2.0,       # 속도오차 -> 페달 P 게인(빠른 가속 램프)
...
def curvature_speed(path, idx: int, seg_end: int,
                    look_m: float = 3.0, base_m: float = 1.0) -> float:  # [G][I]
    """현재 위치 앞쪽(~look_m) 경로의 곡률로 목표 속도 상한을 정한다.
    직선(곡률 0) -> v_straight, 최소회전반경급 곡률 -> v_curve_min 으로 선형 보간.

    [I] 곡률을 '세그먼트(0.4m)별 최대'가 아니라 '~base_m(1m) 구간의 순(net) 방향변화/호길이'의
    최대로 측정한다. Hybrid A* 경로는 이산조향 탓에 직선에서도 매 세그먼트 작은 좌우 꺾임(지그재그)이
    있어, 세그먼트별 최대곡률을 보면 직선조차 곡선으로 오인해 속도를 과하게 깎았다(직선 1.67m/s 묶임).
    순 방향변화는 지그재그가 상쇄돼 실제 굽이만 잡아낸다 — Pure Pursuit+look-ahead 가 지그재그를
    평활 추종하므로 '차가 실제 느끼는 곡률'에도 부합한다.
    """
    if seg_end <= idx:
        return PARAMS["v_straight"]
    max_k = 0.0
    acc_look = 0.0
    i = idx
    while i < seg_end:
        # i 에서 시작하는 ~base_m 구간의 끝 j 를 찾는다
        j = i
        d = 0.0
        while j + 1 <= seg_end and d < base_m:
            x0, y0 = _wp_xy(path[j]); x1, y1 = _wp_xy(path[j + 1])
            d += math.hypot(x1 - x0, y1 - y0)
            j += 1
        if j > i and d > 1e-6:
            ax0, ay0 = _wp_xy(path[i]); ax1, ay1 = _wp_xy(path[i + 1])
            bx0, by0 = _wp_xy(path[j - 1]); bx1, by1 = _wp_xy(path[j])
            h0 = math.atan2(ay1 - ay0, ax1 - ax0)        # 구간 시작 헤딩
            h1 = math.atan2(by1 - by0, bx1 - bx0)        # 구간 끝 헤딩
            max_k = max(max_k, abs(_wrap_to_pi(h1 - h0)) / d)  # 순 방향변화/호길이
        sx0, sy0 = _wp_xy(path[i]); sx1, sy1 = _wp_xy(path[i + 1])
        acc_look += math.hypot(sx1 - sx0, sy1 - sy0)
        if acc_look >= look_m:
            break
        i += 1
...
def path_cross_track(path, seg_s: int, seg_end: int, x: float, y: float) -> float:  # [I]
    """현재 세그먼트 [seg_s..seg_end] 폴리라인까지의 최소 '수직' 거리(참 cross-track error).

    기존엔 cte 를 '가장 가까운 웨이포인트까지의 거리'로 쟀는데, 웨이포인트 간격(~0.4m) 때문에
    잘 추종하는 직선에서도 cte 가 0.4m+ 로 부풀어 cte 감속이 floor 에 박혔다(직선조차 30% 속도).
    선분까지의 수직거리로 바꾸면 직선은 cte≈0 → 감속 거의 없음(고속), 실제 경로 이탈 시에만 감속.
    """
    if seg_end <= seg_s:
        wx, wy = _wp_xy(path[seg_s])
        return math.hypot(x - wx, y - wy)
    best = float("inf")
    for j in range(seg_s, seg_end):
        x0, y0 = _wp_xy(path[j]); x1, y1 = _wp_xy(path[j + 1])
        dx, dy = x1 - x0, y1 - y0
        L2 = dx * dx + dy * dy
        if L2 < 1e-9:
            ...
            t = _clamp(((x - x0) * dx + (y - y0) * dy) / L2, 0.0, 1.0)
...
def lock_stop_decision(x, y, yaw, slot, goal, best_iou, iou_min=0.40):  # [I] 신뢰성 우선 락-스톱
    """정렬됐고 슬롯에 iou_min(0.40) 이상 들어왔으면 즉시 정지(=성공 확정 자세).

    [I] 이전의 '깊이-진입(IoU>=0.45 + 중앙정렬)' 버전은 빡빡해서, 골 근처에서 락이 안 걸려
    차가 멈추질 못했다(롤아웃에선 v<=0.2 도달 실패 → parked_ok 영원히 거짓 → 검증이 '항상'
    실패 → 미검증 폴백 경로로 벽에 박음). 검증된 단순 임계(0.40)로 되돌려 신뢰성을 확보한다.
    (시뮬 성공 임계 0.30보다 0.1 여유 → 정지 직전 미세이동에도 성공 유지.)
    goal/best_iou 인자는 호출부 호환을 위해 유지(미사용).
    반환 (정지?, 현재IoU).
    """
    if slot is None or not orientation_ok(yaw, slot):
        return False, 0.0
    iou = slot_iou(_car_true_corners(x, y, yaw), slot)
    return iou >= iou_min, iou
...
def build_collision_checker(static_map, goal, block_lines=True, inflate=None):  # [E] 재사용 가능 충돌검사 빌더
    ...
    inflate=None 이면 _B_INFLATE(계획용 안전마진 0.25). [I] 롤아웃 검증은 inflate=0.0(시뮬과 동일한
    정확한 차체)으로 호출해야 한다 — 부풀린 차체는 슬롯 깊이진입 시 경계에 닿아 '충돌'로 오판,
    실제론 성공인 경로의 검증을 전부 실패시켰다(→폴백 남발). 검증은 시뮬을 그대로 모사해야 의미가 있다.
    """
    (xmin, xmax_w, ymin, ymax_w, cell, occ, n_rows, n_cols,
     slot_rects, line_rects_obs) = _obstacle_geometry(static_map, goal, block_lines)

    infl = _B_INFLATE if inflate is None else inflate
    _fL, _bL = P_LF + infl, P_LR + infl
    _wH = _B_VEH_WIDTH / 2 + infl
...
def simulate_follow(path, start_state, goal, collision_free, limits, max_steps=12000, slot=None):  # [H]
    ...
    delta = 0.0; idx = 0
    best_iou_roll = 0.0  # [I] 깊이-진입 인지 락-스톱용 IoU 정점 추적
    ...
    for _ in range(max_steps):
        ...
        lock_r, cur_iou_r = (lock_stop_decision(x, y, yaw, slot, goal, best_iou_roll)
                             if slot is not None else (False, 0.0))  # [I]
        if cur_iou_r > best_iou_roll:
            best_iou_roll = cur_iou_r
        steer = 0.0; accel = 0.0; brake = 0.0
        if is_cusp and idx >= seg_end and d_segend < PARAMS["reach_radius"] and abs(v) > PARAMS["stop_speed"]:
            brake = 1.0                                    # 전환점 정지
        elif lock_r:
            brake = 1.0                                    # [I] 깊이-진입(또는 정점통과) -> 정지하여 채점 확정
        else:
            ...
            cte_now = path_cross_track(path, seg_s, seg_end, x, y)  # [I] 참 수직거리
...
def hybrid_astar_plan_bidir(static_map, start_state, goal, block_lines=True, max_iter=None,
                            xy_res=None, step=None):  # [J] 양방향 Hybrid A*
    """양방향 Hybrid A*: start 전방트리와 goal 후방트리(predecessor 확장)를 동시에 키워,
    같은 이산 상태(x,y,yaw)에서 만나면 연결한다. goal 트리가 '슬롯에 정렬돼 빠져나오는' 진입
    기동을 자연히 만들어, 단방향 greedy 가 슬롯 정렬에 실패하던 케이스를 구제한다.
    반환 [[x,y,yaw,direction], ...] (실패 시 []). direction=해당 웨이포인트에 '도착'한 모션 방향.
    """
    if not static_map or not start_state or goal is None:
        return []
    collision_free = build_collision_checker(static_map, goal, block_lines)
    gx, gy, gyaw = goal
    sx = float(start_state['x']); sy = float(start_state['y']); syaw = float(start_state['yaw'])
    xy_res = _B_XY_RES if xy_res is None else float(xy_res)
    step = _B_STEP if step is None else float(step)

    def _key(x, y, yaw):  # 만남/중복 키: 위치+yaw (방향 무관 → 두 트리가 만날 수 있음)
        return (round(x / xy_res), round(y / xy_res), int(round(yaw / _B_YAW_RES)) % _B_N_YAW)

    def hcost(x, y, yaw, tx, ty, tyaw):  # 상대 트리 루트까지 Euclidean greedy 휴리스틱
        return _B_HEURISTIC_WEIGHT * (math.hypot(x - tx, y - ty) + abs(_wrap_to_pi(yaw - tyaw)) * P_L * 0.3)

    f_start = _HybridNode(sx, sy, syaw, 1, 0.0, hcost(sx, sy, syaw, gx, gy, gyaw))
    b_goal = _HybridNode(gx, gy, gyaw, 1, 0.0, hcost(gx, gy, gyaw, sx, sy, syaw))
    open_f = [f_start]; open_b = [b_goal]
    best_f: Dict[tuple, float] = {}; best_b: Dict[tuple, float] = {}
    node_f: Dict[tuple, Any] = {}; node_b: Dict[tuple, Any] = {}   # closed: key -> node (만남 조회용)
    meet = None   # (fnode, bnode)

    def expand_f(cur):  # 전방: 후계(successor) 확장
        for direction in (1, -1):
            d = direction * step
            cusp = _B_CUSP_COST if direction != cur.direction else 0.0
            for steer in _B_STEER_LIST:
                ...
                ng = cur.g + step * (_B_REVERSE_COST if direction < 0 else 1.0) + cusp
                ck = _key(nx, ny, nyaw)
                if ck in best_f and best_f[ck] <= ng:
                    continue
                heapq.heappush(open_f, _HybridNode(nx, ny, nyaw, direction, ng,
                                                   ng + hcost(nx, ny, nyaw, gx, gy, gyaw), cur))

    def expand_b(cur):  # 후방: 선행(predecessor) 확장 — prev 에서 (d,steer)로 cur 에 도달
        for direction in (1, -1):
            d = direction * step
            cusp = _B_CUSP_COST if direction != cur.direction else 0.0
            for steer in _B_STEER_LIST:
                prev_yaw = _wrap_to_pi(cur.yaw - d * math.tan(steer) / P_L)
                prev_x = cur.x - d * math.cos(prev_yaw)
                prev_y = cur.y - d * math.sin(prev_yaw)
                if not collision_free(prev_x, prev_y, prev_yaw):
                    continue
                ng = cur.g + step * (_B_REVERSE_COST if direction < 0 else 1.0) + cusp
                ck = _key(prev_x, prev_y, prev_yaw)
                if ck in best_b and best_b[ck] <= ng:
                    continue
                # direction = prev(child)→cur(parent, 골 쪽) 모션 방향
                heapq.heappush(open_b, _HybridNode(prev_x, prev_y, prev_yaw, direction, ng,
                                                   ng + hcost(prev_x, prev_y, prev_yaw, sx, sy, syaw), cur))

    iters = _B_MAX_ITER if max_iter is None else max_iter
    for _ in range(iters):
        if not open_f and not open_b:
            break
        if open_f:                                       # --- 전방 한 스텝 ---
            cur = heapq.heappop(open_f)
            k = _key(cur.x, cur.y, cur.yaw)
            if not (k in best_f and best_f[k] <= cur.g):
                best_f[k] = cur.g; node_f[k] = cur
                if k in node_b:
                    meet = (cur, node_b[k]); break
                expand_f(cur)
        if open_b:                                       # --- 후방 한 스텝 ---
            cur = heapq.heappop(open_b)
            k = _key(cur.x, cur.y, cur.yaw)
            if not (k in best_b and best_b[k] <= cur.g):
                best_b[k] = cur.g; node_b[k] = cur
                if k in node_f:
                    meet = (node_f[k], cur); break
                expand_b(cur)

    if meet is None:
        print(f"[J] 양방향 A*: 경로 미발견 ({iters}회)")
        return []
    ...
    n = fnode
    while n is not None:
        fwd.append([n.x, n.y, n.yaw, n.direction]); n = n.parent
    fwd.reverse()
    bwd = []                                             # 만남점 → goal (bnode.parent 체인)
    n = bnode.parent
    prev_dir = bnode.direction                           # 만남점→bnode.parent 세그먼트 방향
    while n is not None:
        bwd.append([n.x, n.y, n.yaw, prev_dir])
        prev_dir = n.direction; n = n.parent
    path = fwd + bwd
    print(f"[J] 양방향 A*: 경로 발견 ({len(path)} 웨이포인트)")
    return path
...
class PlannerSkeleton:
    ...
    def __post_init__(self) -> None:
        ...
        self._best_iou = 0.0        # [I] 진입 중 관측한 최대 IoU(정점 통과 감지용)
        self._plan_verified = False # [I] 계획 스레드가 채택한 경로가 롤아웃 검증 통과했는가
        self._path_verified = False # [I] 현재 추종 중 경로의 검증 여부(속도 상한 결정)
    ...
    def set_map(self, map_payload: Dict[str, Any]) -> None:
        ...
        self._best_iou = 0.0        # [I] 새 맵/스테이지마다 IoU 정점 추적 리셋
        self._plan_verified = False
        self._path_verified = False
    ...
    def _compute_control_D(self, obs: Dict[str, Any]):
        ...
        if self.path is None:                   # 아직 경로가 없으면
            ...
            if not self._planning:              # 계획 스레드 시작 (최초 1회)
                ...
                def _worker(gen=gen, map_data=self.map_data, start_state=start_state,
                    ...
                    for goal_c in cands:
                        try:
                            # [I] 롤아웃 검증용 충돌검사기는 inflate=0.0(시뮬과 동일한 정확한 차체).
                            #     부풀린 차체로 검사하면 슬롯 깊이진입을 충돌로 오판해 검증이 전부 실패한다.
                            checkers[id(goal_c)] = build_collision_checker(map_data, goal_c, block_lines=True, inflate=0.0)
                        except Exception:
                    ...
                    def eval_path(p, goal_c):
                        ...
                        # [I] 속도 중심: front/rear 둘 다 성공 인정이므로 방향은 따지지 않고
                        #     '가장 빠른(cusp 적고 짧은)' 경로를 채택한다. 동률이면 먼저 평가하는
                        #     요구방향(cands[0])이 유지돼 orientation 점수도 공짜로 챙긴다.
                    ...
                    # (1b) [J] 검증 실패면 양방향 A* 시도. goal 트리가 슬롯 진입자세를 잘 만들어
                    #      단방향이 검증 못 통과하던 케이스를 구제(training s3 등). 고비용 재탐색보다
                    #      먼저·저렴하게 시도 → 여기서 검증 통과하면 (2a)(2b) 재탐색을 건너뛴다.
                    if verified is None:
                        for goal_c in cands:
                            try:
                                p = hybrid_astar_plan_bidir(map_data, start_state, goal_c,
                                                            block_lines=True, max_iter=120000)
                            except Exception as exc:
                                print(f"[J] 양방향계획 오류: {exc}")
                                p = []
                            if p:
                                eval_path(p, goal_c)

                    # (2a) 검증 실패면 거친격자 '라인회피' 재탐색. [I] 연산속도 위해 30만→15만회로 축소
                    #      (인플레이션 버그 수정 후 대다수가 cheap 단계서 검증 통과 → 재탐색 빈도 자체가 급감).
                    if verified is None:
                        for goal_c in cands:
                            try:
                                p = hybrid_astar_plan(map_data, start_state, goal_c,
                                                      block_lines=True, max_iter=150000)
                            except Exception as exc:
                    ...
                    # (2b) 그래도 검증 실패면 미세 격자(0.2)로 좁은 통로 재탐색. [I] 12만→6만회로 축소.
                    if verified is None:
                        for goal_c in cands:
                            try:
                                p = hybrid_astar_plan(map_data, start_state, goal_c,
                                                      block_lines=True, max_iter=60000,
                                                      xy_res=0.2, step=0.3)
                            except Exception as exc:
                    ...
                    if gen == self._plan_gen:    # 최신 요청일 때만 반영 (맵 교체 race 방지)
                        if chosen is not None:
                            ...
                            self._plan_verified = verified is not None  # [I] 검증 통과 여부
                        else:
            ...
            # [I] 검증 통과 경로는 롤아웃으로 '안전 추종'이 보증됨 → 빠르게. 미검증 폴백 경로는
            #     보증이 없어(추종오차로 벽/라인 긁힘) 보수적 저속으로 따라간다.
            self._path_verified = bool(getattr(self, "_plan_verified", False))
            self.idx = 0
            self.phase = "DRIVE"
        ...
        # (b-2) [I] 깊이-진입 인지 락-스톱: 슬롯에 충분히 깊이(IoU>=0.45) 들어왔거나, IoU 정점을
        #       지나 빠져나가기 시작하면 그 가장 깊은 자세에서 정지한다. (기존 0.40 단일임계는
        #       박스에 얕게 걸친 채 조기정지 → 차가 슬롯 밖으로 삐져나오던 문제를 해결.)
        lock_now, cur_iou = lock_stop_decision(state["x"], state["y"], state.get("yaw", 0.0),
                                               self._target_slot, self.goal, self._best_iou)
        if cur_iou > self._best_iou:
            self._best_iou = cur_iou
        if lock_now:
            self.phase = "STOP"
            return {"steer": 0.0, "accel": 0.0, "brake": 1.0, "gear": "D"}
        ...
        cte_now = path_cross_track(self.path, seg_s, seg_end,
                                   state["x"], state["y"])  # [I] 참 수직거리
        ...
        # [I] 미검증 폴백 경로는 안전 추종 보증이 없어 보수적 저속으로(추종오차 충돌 방지)
        if not getattr(self, "_path_verified", False):
            v_max = min(v_max, PARAMS["v_unverified_cap"])
